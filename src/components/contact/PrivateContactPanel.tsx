...
export function PrivateContactPanel() {
  ...
  const btn =
    "flex items-center justify-center gap-2 rounded-full border border-antique-gold/50 px-6 py-3 font-carved text-[11px] uppercase tracking-[0.18em] text-champagne-gold transition-colors hover:bg-antique-gold hover:text-temple-stone";
  ...
  return (
    <section id="contact" className="grain relative bg-temple-stone py-28">
      ...
      <AnimatePresence>
        {share && (
          <motion.div
            className="fixed inset-0 z-[75] grid place-items-center bg-temple-stone/85 px-6"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
