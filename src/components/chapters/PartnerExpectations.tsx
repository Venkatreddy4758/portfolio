...
export function PartnerExpectations() {
  ...
  return (
    <section className="relative bg-silk-plum py-28">
      <div ref={ref} className="mx-auto max-w-3xl px-6 text-center">
        <div className="mb-8 flex justify-center">
          <LotusMedallion size={40} tone="#B95872" open={inView} />
        </div>
        <h2 className="font-display text-4xl leading-snug text-champagne-gold md:text-5xl">{p.title}</h2>
        <div className="mask-reveal mx-auto mt-10 max-w-2xl" data-shown={inView} style={{ ["--reveal-dur" as string]: "1.6s" }}>
          <p className={`text-lg leading-relaxed text-warm-ivory/85 ${lang === "te" ? "font-telugu" : "font-display text-xl"}`}>
            {p.body}
          </p>
        </div>
      </div>
